...
export default function CleanImageTest() {
  ...
  /* const imgValues = React.useMemo(()=>{
    ...
    const setNewImageLimits = (img:React.RefObject<HTMLImageElement>)=> {
    ...
    img.current
      ...
      : (widthLimit = 0)
      console.log(`Image width and height: ${-widthLimit}:${-heightLimit}`)
      setTopLimit(-heightLimit)
      setLeftLimit(-widthLimit)
      //setSCHeight(img.current ? img.current.height : 0)
      //setSCWidth(img.current ? img.current.width : 0)
      console.log('New Image limits set with topLimit:' + heightLimit + ' and leftLimit:' + widthLimit)
